...
TESTED DENON MODELS
   ...
   telnet command: /goform/formiPhoneAppDirect.xml?COMMAND
*/
...
preferences {
	...
	tiles(scale: 2) {
        ...
        standardTile("input2", "device.tv", width: 2, height: 2, decoration: "flat"){
        	 state "OFF", label: 'Media Server', action: "tv", icon:"st.Electronics.electronics18", backgroundColor:"#FFFFFF",nextState:"ON" 
             state "ON", label: 'Media Server', action: "tv", icon:"st.Electronics.electronics18", backgroundColor: "#53a7c0", nextState:"OFF"             
            }
        standardTile("input3", "device.bd", width: 2, height: 2, decoration: "flat"){
        	state "OFF", label: 'Chromecast', action: "bd", icon:"st.Electronics.electronics8", backgroundColor: "#FFFFFF",nextState:"ON"  
            state "ON", label: 'Chromecast', action: "bd", icon:"st.Electronics.electronics8", backgroundColor: "#53a7c0", nextState:"OFF"              
        	}
        standardTile("input4", "device.dvd", width: 2, height: 2, decoration: "flat"){
        	state "OFF", label: 'WDTV', action: "dvd", icon:"st.Electronics.electronics14", backgroundColor: "#FFFFFF",nextState:"ON"   
            state "ON", label: 'WDTV', action: "dvd", icon:"st.Electronics.electronics14", backgroundColor: "#53a7c0", nextState:"OFF"               
        	}
        ...
        standardTile("input7", "device.favorites", width: 2, height: 2, decoration: "flat"){
        	state "OFF", label: 'IRadio', action: "favorites", icon:"st.Electronics.electronics5", backgroundColor: "#FFFFFF",nextState:"ON"   
            state "ON", label: 'IRadio', action: "favorites", icon:"st.Electronics.electronics5", backgroundColor: "#53a7c0", nextState:"OFF"   
			}               
		standardTile("input10", "device.sound", width: 4, height: 2, decoration: "flat"){
    		state "sMovie", label: '${currentValue}', action:"sMovie", icon:"st.Entertainment.entertainment9", backgroundColor: "#FFFFFF", nextState:"sMusic"
...
def parse(String description) {
    ...
    def favorites() {
        ...
        syncTiles(cmd)
        request("cmd0=PutZone_InputFunction%2F"+cmd)
        request("cmd0=PutZone_InputFunction%2F"+"IRADIO")
    }
    //SOUND MODES
    def sMusic() {
        def cmd = "MUSIC"
    ...
    def q2() { 
        ...
        log.debug "Setting quick input to '${cmd}'"
        syncQTiles(cmd)
       request("cmd0=%20PutUserMode%2FQuick"+cmd)
    }
    def q3() {
        def cmd = "3"
        log.debug "Setting quick input to '${cmd}'"
    ...
    def syncTiles(cmd){
        ...
        if (cmd == "FAVORITES") sendEvent(name: "favorites", value: "ON")	 
            else sendEvent(name: "favorites", value: "OFF")
    }
    def syncQTiles(cmd){
